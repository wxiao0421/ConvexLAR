function [allw1, fullw1] = convexGLAR(x1, y, model, distname, index1, method)

% Function:
%   Calculate the group LAR solution path of general convex function
%
% Arguments:
...
%   method: 'groupl1' (GroupConvexLAR-L1), groupl2' (GroupConvexLAR-L2),
%           'grouplAR' (GroupConvexLAR)
%
% Output:
...
% calculate p
p=size(x,2);
% calculate count, countG, weight, weightG, deriv, derivG, actind, actindG, groupW
INDEX=unique(index);
p2=length(INDEX);
count=zeros(p,1);
for i=1:p
...
while (myloop<=(p2-1))    
    ...
    switch lower(model)
        case 'glm' 
        options=odeset(options,'Events', @(t, f)event3_GLM(t, f, x, y, distname, actind, actindG, p2, index, weightG, method));
        [T,Y,TE,YE,IE]=ode45( @(t, f)mvDirectionG_GLM(x, y, actind, f, distname, actindG, weight, weightG, countG, groupW, index, method, t),[T0, Tmax], wtemp, options);
        case {'panel', 'ada', 'recurrent'}
        options=odeset(options,'Events', @(t, f)event3_EXP(t, f, x, y, model, actind, actindG, p2, index, weightG, method));
        [T,Y,TE,YE,IE]=ode45( @(t, f)mvDirectionG_EXP(x, y, actind, f, model, actindG, weight, weightG, countG, groupW, index, method, t),[T0, Tmax], wtemp, options);
        otherwise
        disp('Unknown model.')
    end
...
switch lower(model)
    ...
    [T,Y]=ode45( @(t, f)mvDirectionG_GLM(x, y, actind, f, distname, actindG, weight, weightG, countG, groupW, index, method, t),[T0, Tmax], wtemp, options);
    case {'panel', 'ada', 'recurrent'}
    options=odeset('RelTol', 1e-10, 'AbsTol', 1e-10);
    [T,Y]=ode45( @(t, f)mvDirectionG_EXP(x, y, actind, f, model, actindG, weight, weightG, countG, groupW, index, method, t),[T0, Tmax], wtemp, options);
    otherwise
    disp('Unknown model.')
end
